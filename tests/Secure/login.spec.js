...
test.describe("Login Tests", () => {
  ...
  test("LOGIN - 01 | @regression Verify the required field error message on login screen", async ({
    page,
  }) => {
  ...
  test("LOGIN - 02 | @regression Verify login with invalid credentials", async ({
    page,
  }) => {
    test.setTimeout(60000);
    await loginPage.navigate();
  ...
  test("LOGIN - 03 | @regression Verify the forgot password error toast when leaving the field empty", async ({
    page,
  }) => {
  ...
  test("LOGIN - 04 | @regression Verify No registered user error toast", async ({
    page,
  }) => {
    test.setTimeout(60000);
    await loginPage.navigate();
  ...
  test("LOGIN - 05 | @regression Verify success toast when navigating to the OTP screen", async ({
    page,
  }) => {
    test.setTimeout(60000);
    await loginPage.navigate();
  ...
  test("LOGIN - 06 | @regression Verify OTP with invalid code", async ({
    page,
  }) => {
    test.setTimeout(60000);
    await loginPage.navigate();
  ...
  test("LOGIN - 07 | @regression Verify incomplete OTP error", async ({
    page,
  }) => {
    test.setTimeout(60000);
    await loginPage.navigate();
  ...
  test("LOGIN - 08 | @regression Verify success toast on resend otp", async ({
    page,
  }) => {
    test.setTimeout(60000);
    await loginPage.navigate();
  ...
  test("LOGIN - 09 | @smoke Verify successful login", async ({
    page,
    context,
  }) => {
    ...
    try {
      await StorageHelper.saveStorage(context, "storageState.json");
    } catch (err) {
